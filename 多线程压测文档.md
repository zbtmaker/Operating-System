        ...
        for (long costTime = 10; costTime <= 20; costTime += 10) {
            ...
            for (int i = 0; i < RPC_CALL_NUM; i++) {
                ThreadPoolManager.getOuterArrayBlockingExecutor().submit(() -> {
                    try {
                        for (int j = 0; j < TOTAL_LOOP; j++) {
                            checkService.parallel(ThreadPoolManager.getInnerArrayBlockingExecutor(), BATCH_USER_NUM, finalCostTime);
                        }
                    } catch (Exception ex) {
                        log.error("", ex);
                    } finally {
...
|400,000|20|LinkedBlockingQueue+CallerRunsPolicy|否|283113|
...
|400,000|10|LinkedBlockingQueue+CallerRunsPolicy|否|125250|
